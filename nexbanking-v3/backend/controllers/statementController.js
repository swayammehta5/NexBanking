...
const downloadPDF = async (req, res) => {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=NexBanking-Statement-${Date.now()}.pdf`
    );
    doc.pipe(res);
    ...
    transactions.forEach((txn, i) => {
      ...
      const balAfter = toNumber(txn.balanceAfter);

      doc.rect(40, rowY - 2, 515, 14).fill(fill).fillColor('#1e293b');
      doc.text(new Date(txn.createdAt).toLocaleDateString(), cols.date, rowY, {
        width: 65,
        continued: true,
      });
      ...
      doc
        .fillColor(color)
        .text(`${sign}$${amt.toFixed(2)}`, cols.amount, rowY, { width: 90, continued: true });
      doc.fillColor('#64748b').text(txn.status, cols.status, rowY, { width: 55, continued: true });
      doc.fillColor('#0f172a').text(`$${balAfter.toFixed(2)}`, cols.balance, rowY, { width: 65 });
      rowY += 15;
      doc.y = rowY;
    });
    ...
    if (transactions.length === 0) {
      doc
        .fontSize(10)
        .fillColor('#94a3b8')
        .text('No transactions found for the selected period.', { align: 'center' });
    }

    doc.moveTo(40, doc.y + 10).lineTo(555, doc.y + 10).strokeColor('#cbd5e1').stroke();
    doc.moveDown(1.5);
    doc.fontSize(9).fillColor('#64748b');
    doc.text(`Closing Balance: $${toNumber(account.balance).toFixed(2)}`, { align: 'right' });
    doc.text('This is a system-generated statement. No signature required.', {
      align: 'center',
    });
...
const downloadCSV = async (req, res) => {
  try {
    ...
    const rows = transactions.map((t) => [
      ...
      toNumber(t.balanceBefore).toFixed(2),
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=NexBanking-Statement-${Date.now()}.csv`
    );
    res.send(csvContent);
  } catch {
