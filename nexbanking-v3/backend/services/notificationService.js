  ...
  try {
    const TEMPLATES = {
      credit: (amount, balance) => ({
        ...
        message: `$${Number(amount).toFixed(2)} has been credited to your account. New balance: $${Number(balance).toFixed(2)}`,
      }),
      debit: (amount, balance) => ({
        title: '💸 Money Debited',
        message: `$${Number(amount).toFixed(2)} has been debited from your account. New balance: $${Number(balance).toFixed(2)}`,
      }),
      transfer: (amount, to) => ({
        title: '↗️ Transfer Sent',
        message: `$${Number(amount).toFixed(2)} transferred to account ${to}.`,
      }),
      transfer_received: (amount, from) => ({
        title: '↙️ Transfer Received',
        message: `$${Number(amount).toFixed(2)} received from account ${from}.`,
      }),
      ...
      fraud_alert: (amount, reason) => ({
        ...
        message: `A transaction of $${Number(amount).toFixed(2)} was flagged: ${reason}`,
      }),
    };
